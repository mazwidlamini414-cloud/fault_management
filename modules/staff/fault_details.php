<?php
// ── Bootstrap: session + auth + DB must come before any output ────────────────
...
if (!$fault_id) {
    header('Location: assigned_faults.php');
    exit();
}

// ── Helper: fetch fault row (verifies it belongs to this technician) ──────────
function fetch_fault($conn, $fault_id, $tech_id) {
...
$fault = fetch_fault($conn, $fault_id, $tech_id);

if (!$fault) {
    $_SESSION['toast'] = ['msg' => 'Fault not found or not assigned to you.', 'type' => 'error'];
    header('Location: assigned_faults.php');
    exit();
}

// ── Handle POST / GET actions — ALL redirects happen here, before any output ──
if ($_SERVER['REQUEST_METHOD'] === 'POST' || isset($_GET['action'])) {
    ...
    if ($action === 'start' && $fault['STATUS'] === 'Assigned') {
        ...
        mysqli_query($conn, "INSERT INTO work_log (ASSIGN_ID, EMP_ID, LOG_TYPE, ACTION_TAKEN, HOURS_SPENT)
                             VALUES ({$fault['ASSIGN_ID']}, $tech_id, 'Start', 'Work started by technician', 0)");
        mysqli_query($conn, "INSERT INTO notifications (user_id, user_type, title, message)
                             VALUES (1, 'Admin', 'Work Started', 'Technician " . mysqli_real_escape_string($conn, $tech_name) . " started work on fault #$fault_id')");
        $_SESSION['toast'] = ['msg' => 'Work started. Status updated to In Progress.', 'type' => 'success'];
        header("Location: fault_details.php?id=$fault_id");
        exit();
    }

    if ($action === 'complete' && $fault['STATUS'] === 'In Progress') {
        if (!$note) {
            $_SESSION['toast'] = ['msg' => 'Please add a completion note.', 'type' => 'error'];
            header("Location: fault_details.php?id=$fault_id");
            exit();
        }
        ...
        mysqli_query($conn, "INSERT INTO work_log (ASSIGN_ID, EMP_ID, LOG_TYPE, ACTION_TAKEN, HOURS_SPENT)
                             VALUES ({$fault['ASSIGN_ID']}, $tech_id, 'Complete', '$note_esc', 0)");
        mysqli_query($conn, "INSERT INTO notifications (user_id, user_type, title, message)
                             VALUES (1, 'Admin', 'Fault Completed', 'Fault #$fault_id marked completed by " . mysqli_real_escape_string($conn, $tech_name) . "')");
        mysqli_query($conn, "INSERT INTO notifications (user_id, user_type, title, message)
                             VALUES ({$fault['CLIENT_ID']}, 'Client', 'Fault Resolved', 'Your fault #$fault_id has been resolved. Please verify and confirm.')");
    ...
    if ($action === 'note' && $note) {
        $note_esc = mysqli_real_escape_string($conn, $note);
        mysqli_query($conn, "INSERT INTO work_log (ASSIGN_ID, EMP_ID, LOG_TYPE, ACTION_TAKEN, HOURS_SPENT)
                             VALUES ({$fault['ASSIGN_ID']}, $tech_id, 'Note', '$note_esc', 0)");
        $_SESSION['toast'] = ['msg' => 'Note added to fault timeline.', 'type' => 'success'];
        header("Location: fault_details.php?id=$fault_id");
        exit();
    }

    // Re-fetch after any non-redirecting action
    $fault = fetch_fault($conn, $fault_id, $tech_id);
}

// ── Fetch work log / timeline ─────────────────────────────────────────────────
...
// ── Page meta (used by tech_header.php) ──────────────────────────────────────
$page_title    = 'Fault Details';
$page_subtitle = 'Fault #' . $fault_id;

// ── NOW it is safe to include the header (outputs HTML) ──────────────────────
...
<div class="page-head">
  ...
  <div style="display:flex;gap:.5rem;flex-wrap:wrap">
    ...
    <form method="POST" style="display:inline">
      ...
      <button type="submit" class="btn btn-primary btn-sm" onclick="return confirm('Start work on this fault?')">
      ...
      <div class="timeline">
        ...
        <div class="tl-item">
          ...
          <div class="tl-content">
            ...
            <div class="tl-meta">
              <?= date('d M Y H:i', strtotime($log['LOG_DATE'])) ?> &bull;
              <?= htmlspecialchars($log['FULL_NAME'] ?? 'Technician') ?>
              <?= $log['HOURS_SPENT'] > 0 ? ' &bull; ' . $log['HOURS_SPENT'] . 'h' : '' ?>
            </div>
