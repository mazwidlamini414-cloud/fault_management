...
// 1. SECURITY CHECK: 
// If the user is NOT a Technician, send them back to the staff login.
// This prevents the redirect loop for authorized users.
...
// 2. DATA SETUP:
// These variables will now be available to your dashboard.
$tech_id    = $_SESSION['user_id'] ?? $_SESSION['emp_id']; 
...
$notif_res = mysqli_query($conn, "SELECT COUNT(*) as cnt FROM notifications WHERE user_id = $tech_id AND user_type = 'Employee' AND is_read = 0");
$notif_count = mysqli_fetch_assoc($notif_res)['cnt'] ?? 0;

// Unread messages count
$msg_res = mysqli_query($conn, "SELECT COUNT(*) as cnt FROM unified_messages WHERE to_id = $tech_id AND to_type = 'Employee' AND is_read = 0");
...
$nav_items = [
    ...
    ['page' => 'messages',          'icon' => 'message-circle',    'label' => 'Messages'],
...
<aside class="sidebar">
  ...
  <nav class="sidebar-nav">
    ...
    <a href="<?= $item['page'] ?>.php" class="nav-item <?= $current_page === $item['page'] ? 'active' : '' ?>">
      ...
      <?= $item['label'] ?>
      <?php if ($item['page'] === 'notifications' && $notif_count > 0): ?>
        <span class="nav-badge"><?= $notif_count ?></span>
      <?php elseif ($item['page'] === 'messages' && $msg_count > 0): ?>
