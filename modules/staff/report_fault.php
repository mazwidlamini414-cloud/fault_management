...
if (isset($_GET['action'])) {
    ...
    if ($action === 'submit_fault' && $_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        // reported_fault fields (matching DB column order):
        // REP_FAULT_ID (auto), CLIENT_ID (session), CLIENT_PROD_ID, FAULT_ID,
        // REPORT_DATE, STATUS, PRIORITY, REPORTED_BY, DESCRIPTION

        $fault_ref        = $safe($_POST['fault_ref']         ?? $ref_num);
        $fault_title      = $safe($_POST['fault_title']       ?? '');
        $description      = $safe($_POST['description']       ?? '');
        $fault_category   = $safe($_POST['fault_category']    ?? '');
        ...
        $users_affected   = max(0, (int)($_POST['users_affected'] ?? 1));
        $fault_location   = $safe($_POST['fault_location']    ?? '');
        $dept_branch      = $safe($_POST['dept_branch']       ?? '');
        $contact_method   = $safe($_POST['contact_method']    ?? 'Email');
        $service_visit    = $safe($_POST['service_visit']     ?? 'No');
        $reported_by      = $safe($_POST['reported_by']       ?? $contact_person);

        // Validation
        $errors = [];
        if (strlen($fault_title) < 5)       $errors[] = 'Fault title must be at least 5 characters.';
        if (strlen($description) < 20)      $errors[] = 'Fault description must be at least 20 characters.';
        if (empty($fault_category))         $errors[] = 'Please select a fault category.';
        if (empty($equipment_type))         $errors[] = 'Please enter the equipment type.';
        if (strtotime($fault_date) > time()) $errors[] = 'Fault date cannot be in the future.';
        if (!preg_match('/^\d{4}-\d{2}-\d{2}$/', $fault_date)) $errors[] = 'Invalid fault date format.';
        if (!preg_match('/^\d{2}:\d{2}$/', $fault_time))       $errors[] = 'Invalid fault time format.';
        ...
        // Build full report_date datetime
        $report_date = date('Y-m-d H:i:s');
        $fault_datetime = $fault_date . ' ' . $fault_time . ':00';

        // Build extended description with all extra fields
        $full_desc = "FAULT REFERENCE: $fault_ref\n";
        $full_desc .= "FAULT TITLE: $fault_title\n";
        $full_desc .= "CATEGORY: $fault_category\n";
        ...
        // DB column order: CLIENT_ID, CLIENT_PROD_ID, FAULT_ID, REPORT_DATE, STATUS, PRIORITY, REPORTED_BY, DESCRIPTION
        $stmt = $conn->prepare(
            "INSERT INTO reported_fault (CLIENT_ID, REPORT_DATE, STATUS, PRIORITY, REPORTED_BY, DESCRIPTION)
             VALUES (?, ?, 'Pending', ?, ?, ?)"
        );
        $stmt->bind_param("issss", $client_id, $report_date, $priority, $reported_by, $full_desc);
        ...
        if ($stmt->execute()) {
            ...
            echo json_encode([
                'success'  => true,
                'message'  => "Fault report submitted successfully! Reference: <strong>$fault_ref</strong>. Our admin team will review and assign a technician promptly.",
                'id'       => $new_id,
                'ref'      => $fault_ref,
...
<style>
/* ══════════════════════════════════════════════════════════════════════
   BUSIQUIP — CSS DESIGN SYSTEM (matches client_portalfinal.php exactly)
══════════════════════════════════════════════════════════════════════ */
:root {
...
.o1{width:500px;height:500px;top:-180px;left:-180px;background:radial-gradient(circle,var(--burg),transparent);animation-delay:0s}
...
/* ── Ticker ── */
.ticker{
  position:fixed;top:0;left:0;right:0;height:26px;z-index:3000;
  background:linear-gradient(90deg,var(--burg),#6B0000,var(--burg));
  overflow:hidden;display:flex;align-items:center;
}
.ticker-inner{
  display:flex;gap:60px;animation:tick 30s linear infinite;white-space:nowrap;
...
/* ── Header ── */
header{
  position:fixed;top:26px;left:0;right:0;height:68px;z-index:1500;
...
.brand-ic{
  ...
  box-shadow:0 0 18px var(--burg-g);animation:spin-s 14s linear infinite;
}
...
.brand-nm{
  ...
  -webkit-background-clip:text;-webkit-text-fill-color:transparent;letter-spacing:.05em;
}
...
.h-badge{
  ...
  padding:5px 12px;border-radius:var(--r);font-family:var(--fm);font-size:11px;color:var(--em);
}
.h-badge small{font-size:9px;color:var(--t2);display:block;font-family:var(--fb)}

.hb{
  width:38px;height:38px;border-radius:50%;border:1px solid var(--bor);
  background:var(--gl);color:var(--t2);font-size:15px;
...
.btn-back{
  ...
  border-radius:var(--r);border:1px solid var(--bor);background:var(--gl);
  color:var(--t2);font-size:13px;font-weight:500;transition:var(--tr);
}
...
.page-hero{
  ...
  background:linear-gradient(135deg,rgba(139,0,0,.15),rgba(232,184,75,.08));
  border:1px solid var(--borh);border-radius:var(--rl);
  position:relative;overflow:hidden;
}
.page-hero::before{
  content:'';position:absolute;inset:0;
  background:linear-gradient(135deg,rgba(139,0,0,.06),transparent,rgba(232,184,75,.04));
  pointer-events:none;
}
.hero-icon{
  ...
  box-shadow:0 12px 36px var(--burg-g);animation:pulse-ic 3s ease-in-out infinite;
}
...
.ref-display{
  ...
  background:var(--gl);border:1px solid var(--borh);border-radius:var(--r);
  padding:8px 18px;font-family:var(--fm);font-size:13px;color:var(--gold);
  letter-spacing:.08em;
}
...
.step-num{
  width:36px;height:36px;border-radius:50%;border:2px solid var(--t3);
  background:var(--bg1);color:var(--t3);font-family:var(--fm);font-size:13px;
  display:flex;align-items:center;justify-content:center;transition:var(--tr);position:relative;z-index:1;
}
...
.card{
  ...
  margin-bottom:20px;overflow:hidden;transition:var(--tr);
...
.card-header{
  ...
  border-bottom:1px solid var(--bor);cursor:pointer;user-select:none;
  transition:var(--tr);
}
.card-header:hover{background:var(--gl)}
.sec-icon{
  width:42px;height:42px;border-radius:12px;
...
.card-header .badge-req{
  font-size:9px;font-family:var(--fm);padding:3px 8px;border-radius:6px;
  background:rgba(239,68,68,.15);color:var(--dan);border:1px solid rgba(239,68,68,.3);
  letter-spacing:.05em;text-transform:uppercase;
}
.card-header .badge-auto{
  font-size:9px;font-family:var(--fm);padding:3px 8px;border-radius:6px;
  background:rgba(16,185,129,.12);color:var(--em);border:1px solid rgba(16,185,129,.3);
  letter-spacing:.05em;text-transform:uppercase;
}
.card-toggle{color:var(--t3);transition:transform .3s}
...
label{
  font-size:12px;font-weight:600;color:var(--t2);letter-spacing:.06em;
...
label .auto-tag{
  ...
  background:rgba(16,185,129,.12);color:var(--em);border:1px solid rgba(16,185,129,.25);
  letter-spacing:.04em;
}
...
input:focus,select:focus,textarea:focus{
  ...
  box-shadow:0 0 0 3px var(--gold-p);background:var(--bg3);
}
input[readonly],select[disabled]{
  background:var(--bg1) !important;color:var(--t2) !important;
  border-color:rgba(255,255,255,.06) !important;cursor:not-allowed;
}
...
/* field error */
.field-err{
  font-size:11px;color:var(--dan);display:none;
  align-items:center;gap:5px;margin-top:3px;
}
.fg.has-error input,.fg.has-error select,.fg.has-error textarea{
  border-color:var(--dan);box-shadow:0 0 0 3px rgba(239,68,68,.15);
}
.fg.has-error .field-err{display:flex}
.fg.has-ok input,.fg.has-ok select,.fg.has-ok textarea{
  border-color:var(--em);box-shadow:0 0 0 3px rgba(16,185,129,.1);
}

/* radio / toggle group */
.toggle-group{display:flex;gap:10px;flex-wrap:wrap}
.toggle-opt{
  flex:1;min-width:80px;padding:10px 14px;border-radius:var(--r);
  border:1px solid var(--bor);background:var(--bg2);
  color:var(--t2);font-size:13px;font-weight:500;text-align:center;
...
.sev-pill{
  flex:1;min-width:90px;padding:14px 10px;border-radius:var(--r);
  border:2px solid var(--bor);background:var(--bg2);
  text-align:center;cursor:pointer;transition:var(--tr);user-select:none;
}
...
/* ── File Upload ── */
.upload-zone{
  border:2px dashed var(--bor);border-radius:var(--rl);
  padding:30px;text-align:center;cursor:pointer;transition:var(--tr);
  background:var(--gl);position:relative;overflow:hidden;
}
.upload-zone:hover,.upload-zone.drag-over{
  border-color:var(--gold);background:var(--gold-p);
}
...
.type-badge{
  font-size:10px;font-family:var(--fm);padding:3px 8px;border-radius:6px;
  background:var(--gl);border:1px solid var(--bor);color:var(--t3);
}

.preview-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(100px,1fr));gap:10px;margin-top:14px}
.preview-item{
  position:relative;border-radius:var(--r);overflow:hidden;
  border:1px solid var(--bor);background:var(--bg2);aspect-ratio:1;
  display:flex;align-items:center;justify-content:center;flex-direction:column;
  font-size:11px;color:var(--t2);gap:4px;
  animation:fadeIn .3s ease;
}
.preview-item img,.preview-item video{width:100%;height:100%;object-fit:cover}
.preview-item .file-icon{font-size:28px}
...
.preview-item .rem-btn{
  position:absolute;top:4px;right:4px;width:20px;height:20px;
  border-radius:50%;background:var(--dan);color:#fff;font-size:11px;
...
/* ── Info box ── */
.ib{
  display:flex;gap:12px;padding:14px 18px;border-radius:var(--r);
...
/* ── Submit area ── */
.submit-area{
  background:linear-gradient(135deg,rgba(139,0,0,.12),rgba(232,184,75,.07));
  border:1px solid var(--borh);border-radius:var(--rl);
  padding:30px 28px;text-align:center;margin-top:28px;
}
...
.check-item{
  display:flex;align-items:center;gap:6px;
  font-size:12px;color:var(--t2);background:var(--gl);
  border:1px solid var(--bor);border-radius:var(--r);padding:6px 12px;
}
...
.btn-submit{
  ...
  background:linear-gradient(135deg,var(--burg),var(--gold));
  color:#fff;font-family:var(--fh);font-size:16px;font-weight:800;
  letter-spacing:.04em;transition:var(--tr);
  box-shadow:0 8px 28px var(--burg-g);
  display:inline-flex;align-items:center;gap:10px;
}
.btn-submit:hover:not(:disabled){
  transform:translateY(-2px);
  box-shadow:0 12px 36px var(--burg-g),0 0 40px var(--gold-p);
}
.btn-submit:disabled{
...
.btn-cancel{
  ...
  background:var(--gl);border:1px solid var(--bor);color:var(--t2);
  font-size:14px;font-weight:500;transition:var(--tr);
...
.al{
  ...
  border-radius:var(--r);font-size:13px;font-weight:500;pointer-events:all;
  backdrop-filter:var(--blur);box-shadow:var(--sh);min-width:280px;max-width:380px;
  animation:slide-in .35s cubic-bezier(.34,1.56,.64,1);transition:opacity .4s;
}
...
#success-screen{
  ...
  background:rgba(7,12,20,.97);backdrop-filter:var(--blur);
  align-items:center;justify-content:center;padding:24px;
}
#success-screen.show{display:flex}
.success-card{
  background:var(--bg2);border:1px solid var(--borh);border-radius:var(--rl);
  padding:48px 40px;text-align:center;max-width:560px;width:100%;
...
.success-icon{
  ...
  font-size:36px;color:#fff;box-shadow:0 16px 48px rgba(16,185,129,.35);
  animation:success-bounce .8s ease .3s both;
}
...
.success-ref{
  background:var(--bg1);border:1px solid var(--borh);border-radius:var(--r);
  padding:14px 24px;margin:20px 0;font-family:var(--fm);font-size:18px;
  color:var(--gold);letter-spacing:.1em;
}
.success-steps{
  display:flex;gap:12px;margin:20px 0;text-align:left;
}
.ss-item{
  flex:1;background:var(--gl);border:1px solid var(--bor);border-radius:var(--r);
  padding:14px;font-size:12px;color:var(--t2);line-height:1.5;
}
...
.btn-home{
  ...
  background:linear-gradient(135deg,var(--burg),var(--gold));
  color:#fff;font-family:var(--fh);font-size:14px;font-weight:700;transition:var(--tr);
}
.btn-home:hover{transform:translateY(-2px);box-shadow:0 8px 24px var(--burg-g)}
.btn-new{
  padding:12px 28px;border-radius:var(--r);
  background:var(--gl);border:1px solid var(--bor);color:var(--t2);font-size:14px;transition:var(--tr);
}
.btn-new:hover{border-color:var(--gold);color:var(--gold)}

/* ── Decorative live particles ── */
.particles{position:fixed;inset:0;pointer-events:none;z-index:0;overflow:hidden}
.particle{
  position:absolute;width:3px;height:3px;border-radius:50%;
  background:var(--gold);opacity:0;animation:float-up 8s linear infinite;
}
@keyframes float-up{
...
.data-chip{
  ...
  background:var(--gl);border:1px solid var(--bor);border-radius:var(--r);
  padding:8px 14px;font-size:12px;font-family:var(--fm);color:var(--t2);
  flex:1;min-width:140px;
}
.data-chip .dc-dot{width:7px;height:7px;border-radius:50%;background:var(--em);animation:blink 2s ease-in-out infinite}
...
.autosave{
  ...
  padding:6px 12px;border-radius:8px;background:var(--gl);border:1px solid var(--bor);
}
...
<div class="ticker">
  <div class="ticker-inner" id="tickerInner">
    ...
    <span>🔧 Submit Your Fault Report — We Respond Within 2 Hours</span>
    <span>📋 All Reports Are Reviewed By Our Admin Team Immediately</span>
    <span>🛡️ Secure &amp; Encrypted Submission</span>
    <span>🇸🇿 Serving Eswatini's Businesses With Excellence</span>
    <span>⚡ BUSIQUIP ESWATINI — FAULT MANAGEMENT SYSTEM</span>
    <span>🔧 Submit Your Fault Report — We Respond Within 2 Hours</span>
    <span>📋 All Reports Are Reviewed By Our Admin Team Immediately</span>
    <span>🛡️ Secure &amp; Encrypted Submission</span>
...
<header>
  ...
  <div class="h-right">
    <div class="h-badge">
      <div class="dc-dot" style="display:inline-block;width:7px;height:7px;border-radius:50%;background:var(--em);margin-right:5px;animation:blink 2s ease-in-out infinite"></div>
      <?= htmlspecialchars($company_name) ?>
      <small>Session Active</small>
    </div>
    <div class="autosave" id="autosave-ind">
      <div class="as-dot"></div>
      <span id="autosave-txt">Draft saved</span>
    </div>
    <button class="hb" onclick="toggleTheme()" title="Toggle Light/Dark Mode"><i class="fas fa-moon" id="themeIcon"></i></button>
    <a href="client_portalfinal.php" class="btn-back"><i class="fas fa-arrow-left"></i> Back to Portal</a>
  </div>
</header>
...
<div id="success-screen">
  <div class="success-card">
    ...
    <p>Your fault report has been successfully submitted to the Busiquip admin team. You will be contacted promptly.</p>
    <div class="success-ref" id="success-ref-num">BQ-2024-00000</div>
    <p style="font-size:12px;color:var(--t3)">Save this reference number for tracking your fault status.</p>
    <div class="success-steps">
      ...
      <div class="ss-item"><strong>02 — Technician Assigned</strong>A technician will be dispatched based on your fault severity and location.</div>
      <div class="ss-item"><strong>03 — You'll Be Notified</strong>Updates will be sent via your preferred contact method.</div>
    </div>
    <div class="success-btns">
      <a href="client_portalfinal.php" class="btn-home"><i class="fas fa-home"></i> Return to Portal</a>
...
<div class="page-wrap">
  ...
  <div class="page-hero">
    ...
    <p>Complete the form below with as much detail as possible. Our team will review your report immediately and assign a qualified technician.</p>
  ...
  <div class="progress-steps">
    ...
    <div class="step" id="step-4">
      ...
      <div class="step-label">Severity</div>
    </div>
  ...
  <div class="data-strip">
    ...
    <div class="data-chip"><span class="dc-dot"></span> Date <span class="dc-val" id="strip-date"><?= date('d M Y') ?></span></div>
    <div class="data-chip"><span class="dc-dot"></span> Time <span class="dc-val" id="strip-time"><?= date('H:i') ?></span></div>
    <div class="data-chip"><span class="dc-dot"></span> Client <span class="dc-val"><?= htmlspecialchars(substr($company_name,0,18)) ?></span></div>
  </div>
  ...
  <form id="faultForm" novalidate>
    ...
    <!-- ══════════════════════════════════════════════════════
         SECTION 1 — CLIENT INFORMATION (AUTO-FILLED)
    ══════════════════════════════════════════════════════ -->
    <div class="card" id="card-client">
      <div class="card-header" onclick="toggleCard('card-client')">
        <div class="sec-icon ic-client"><i class="fas fa-building"></i></div>
      ...
      <div class="card-body">
        <div class="ib ib-auto">
          ...
          <div>These fields are automatically populated from your registered account session. Contact support if any information is incorrect.</div>
        </div>

        <!-- DB order: CLIENT_ID, COMPANY_NAME, COMPANY_PHONE, COMPANY_EMAIL, COMPANY_ADDRESS, CONTACT_PERSON_NAME, CLIENT_TYPE -->
        <div class="fr">
          <div class="fg">
            <label><i class="fas fa-id-badge"></i> Client ID <span class="auto-tag">SESSION</span></label>
            <input type="text" name="client_id_display" value="CLIENT-<?= str_pad($client_id, 4, '0', STR_PAD_LEFT) ?>" readonly>
        ...
        <div class="fr">
          <div class="fg">
            ...
            <input type="text" name="company_phone" value="<?= $company_phone ?>" readonly>
          </div>
          <div class="fg">
            <label><i class="fas fa-envelope"></i> Company Email <span class="auto-tag">SESSION</span></label>
            <input type="text" name="company_email" value="<?= $company_email ?>" readonly>
          </div>
        </div>

        <div class="fg">
          <label><i class="fas fa-map-marker-alt"></i> Physical Address <span class="auto-tag">SESSION</span></label>
          <input type="text" name="company_address" value="<?= $company_address ?>" readonly>
        </div>

        <div class="fr">
          <div class="fg">
            <label><i class="fas fa-user-tie"></i> Contact Person Name <span class="auto-tag">SESSION</span></label>
            <input type="text" name="contact_person" value="<?= $contact_person ?>" readonly>
          </div>
          <div class="fg">
            <label><i class="fas fa-tags"></i> Client Type <span class="auto-tag">SESSION</span></label>
            <input type="text" name="client_type" value="<?= $client_type_v ?>" readonly>
          </div>
        </div>

        <div class="fg">
          <label><i class="fas fa-code-branch"></i> Department / Branch <span class="tip">(optional — which department is affected?)</span></label>
          <input type="text" name="dept_branch" id="dept_branch" placeholder="e.g. IT Department, Finance Branch, Head Office…" maxlength="100">
        </div>

        <div class="fg">
          <label><i class="fas fa-user"></i> Reported By <span class="req-star">*</span></label>
          <input type="text" name="reported_by" id="reported_by" value="<?= $contact_person ?>" placeholder="Name of person reporting this fault" maxlength="150" required>
          <div class="field-err"><i class="fas fa-circle-exclamation"></i> Please enter the reporter's name.</div>
        </div>
      </div>
    </div>

    <!-- ══════════════════════════════════════════════════════
         SECTION 2 — FAULT INFORMATION
    ══════════════════════════════════════════════════════ -->
    <div class="card" id="card-fault">
      <div class="card-header" onclick="toggleCard('card-fault')">
      ...
      <div class="card-body">
        <div class="ib ib-info">
          ...
          <div>Be as specific as possible. Include error messages, symptoms, when it started, and anything unusual you observed.</div>
        </div>

        <!-- DB: DESCRIPTION (fault_title is prepended), FAULT_ID (mapped to category) -->
        <div class="fg">
          <label><i class="fas fa-heading"></i> Fault Title <span class="req-star">*</span></label>
          <input type="text" name="fault_title" id="fault_title" placeholder="Brief title — e.g. Printer not printing, Network connectivity lost…" maxlength="120" required>
          <div class="char-count" id="title-count">0 / 120</div>
          <div class="field-err"><i class="fas fa-circle-exclamation"></i> Fault title must be at least 5 characters.</div>
        </div>

        <div class="fg">
          <label><i class="fas fa-align-left"></i> Fault Description <span class="req-star">*</span></label>
          <textarea name="description" id="description" placeholder="Describe the fault in detail. Include: what happened, when it started, error messages shown, steps already tried, any recent changes…" maxlength="2000" required></textarea>
          <div class="char-count" id="desc-count">0 / 2000</div>
          <div class="field-err"><i class="fas fa-circle-exclamation"></i> Description must be at least 20 characters.</div>
        </div>

        <div class="fr">
          <div class="fg">
            <label><i class="fas fa-list"></i> Fault Category <span class="req-star">*</span></label>
            <select name="fault_category" id="fault_category" required>
              ...
              <option value="Storage / Data Loss">Storage / Data Loss</option>
            ...
            <div class="field-err"><i class="fas fa-circle-exclamation"></i> Please select a fault category.</div>
          </div>
          <div class="fg">
        ...
        <!-- DB: REPORT_DATE — fault_date + fault_time = actual occurrence -->
        <div class="fr">
          <div class="fg">
            <label><i class="fas fa-calendar"></i> Date Fault Occurred <span class="req-star">*</span></label>
            <input type="date" name="fault_date" id="fault_date" value="<?= $today ?>" max="<?= $today ?>" required>
            <div class="field-err"><i class="fas fa-circle-exclamation"></i> Please select the date the fault occurred (cannot be future).</div>
          </div>
          <div class="fg">
            ...
            <div class="field-err"><i class="fas fa-circle-exclamation"></i> Please select the time of fault.</div>
          </div>
        </div>

        <div class="fr">
          <div class="fg">
            <label><i class="fas fa-map-pin"></i> Location of Fault</label>
            <input type="text" name="fault_location" id="fault_location" placeholder="Building, floor, room, area…" maxlength="150">
          </div>
          <div class="fg">
            <label><i class="fas fa-users"></i> Number of Users Affected</label>
            <input type="number" name="users_affected" id="users_affected" value="1" min="1" max="9999" placeholder="1">
          </div>
        </div>

        <div class="fr">
          <div class="fg">
            <label><i class="fas fa-power-off"></i> Is Equipment Operational?</label>
            <div class="toggle-group">
              <div class="toggle-opt selected-yes" data-group="operational" data-val="Yes" onclick="selectToggle(this,'operational')">✅ Yes — Partially</div>
              <div class="toggle-opt" data-group="operational" data-val="No" onclick="selectToggle(this,'operational')">❌ No — Completely Down</div>
            </div>
            <input type="hidden" name="is_operational" id="is_operational" value="Yes">
          </div>
          <div class="fg">
            <label><i class="fas fa-history"></i> Has Problem Occurred Before?</label>
            <div class="toggle-group">
              <div class="toggle-opt selected-no" data-group="recurrence" data-val="No" onclick="selectToggle(this,'recurrence')">❌ No — First Time</div>
              <div class="toggle-opt" data-group="recurrence" data-val="Yes" onclick="selectToggle(this,'recurrence')">⚠️ Yes — Recurring</div>
            </div>
        ...
        <div class="fg">
          <label><i class="fas fa-truck"></i> Service Visit Required?</label>
          <div class="toggle-group">
            <div class="toggle-opt selected-neutral" data-group="service_visit" data-val="Yes" onclick="selectToggle(this,'service_visit')">🔧 Yes — On-Site Visit</div>
            <div class="toggle-opt" data-group="service_visit" data-val="No" onclick="selectToggle(this,'service_visit')">💻 No — Remote Support</div>
            <div class="toggle-opt" data-group="service_visit" data-val="Unsure" onclick="selectToggle(this,'service_visit')">❓ Not Sure</div>
          </div>
          <input type="hidden" name="service_visit" id="service_visit" value="Yes">
        </div>
      </div>
    </div>
    ...
    <div class="card" id="card-equip">
      <div class="card-header" onclick="toggleCard('card-equip')">
        <div class="sec-icon ic-equip"><i class="fas fa-desktop"></i></div>
        <h3>Equipment Details</h3>
        <i class="fas fa-chevron-down card-toggle"></i>
      </div>
      <div class="card-body">
        <!-- DB: CLIENT_PROD_ID (serial_number links to client_product) -->
        <div class="fr">
          <div class="fg">
            ...
            <select name="equipment_type" id="equipment_type" required>
              ...
              <option value="Telephone / PBX">Telephone / PBX</option>
              <option value="Other">Other</option>
            </select>
            <div class="field-err"><i class="fas fa-circle-exclamation"></i> Please select the equipment type.</div>
          </div>
          <div class="fg">
            <label><i class="fas fa-tag"></i> Brand / Model</label>
            <input type="text" name="brand_model" id="brand_model" placeholder="e.g. HP LaserJet Pro M404, Dell Latitude 5520…" maxlength="120">
          </div>
        </div>

        <div class="fr">
          <div class="fg">
            <label><i class="fas fa-barcode"></i> Serial Number / Asset Number</label>
            <input type="text" name="serial_number" id="serial_number" placeholder="Device serial number or company asset tag" maxlength="100">
          </div>
          <div class="fg">
            <label><i class="fas fa-box"></i> Registered Equipment <span class="tip">(if registered in system)</span></label>
    ...
    <!-- ══════════════════════════════════════════════════════
         SECTION 4 — FAULT SEVERITY
    ══════════════════════════════════════════════════════ -->
    <div class="card" id="card-severity">
      <div class="card-header" onclick="toggleCard('card-severity')">
      ...
      <div class="card-body">
        <div class="ib ib-warn">
          ...
          <div>Select the severity that best matches the impact. Critical faults receive immediate priority response.</div>
        </div>
        <!-- DB: PRIORITY -->
        ...
        <div class="sev-group">
          <div class="sev-pill" data-val="Low" onclick="selectSeverity(this)">
            ...
            <div class="sev-desc">Minor inconvenience, workaround available</div>
          </div>
          ...
          <div class="sev-pill" data-val="Critical" onclick="selectSeverity(this)">
            ...
            <div class="sev-desc">Complete system/business stoppage</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="card" id="card-attach">
      <div class="card-header" onclick="toggleCard('card-attach')">
      ...
      <div class="card-body">
        <div class="ib ib-info">
          <i class="fas fa-info-circle"></i>
          <div>Upload photos, videos, or documents that help describe the fault. Max 20MB per file. Photos and screenshots are especially helpful.</div>
        </div>
        ...
        <div class="upload-zone" id="uploadZone">
          ...
          <p>Upload photos, videos, and documents related to the fault</p>
          <div class="upload-types">
            ...
            <span class="type-badge">Max 20MB</span>
          </div>
        </div>
    ...
    <div class="submit-area">
      ...
      <p>Your fault report will be submitted securely and immediately appear on the Admin Dashboard for review and technician assignment.</p>

      <div class="checklist">
        <div class="check-item"><i class="fas fa-check ci"></i> Auto-assigned reference number</div>
        ...
        <div class="check-item"><i class="fas fa-check ci"></i> Activity log updated</div>
      ...
      <div id="validation-summary" style="display:none;margin-bottom:20px;padding:14px 18px;border-radius:var(--r);background:rgba(239,68,68,.1);border:1px solid rgba(239,68,68,.3);color:var(--dan);font-size:13px;text-align:left">
        <strong><i class="fas fa-triangle-exclamation"></i> Please fix the following:</strong>
        <ul id="val-list" style="margin:8px 0 0 18px;line-height:2"></ul>
      </div>

      <button type="button" class="btn-submit" id="submitBtn" onclick="submitFault()">
        <i class="fas fa-paper-plane"></i>
        Submit Fault Report
      </button>

      <br>
      <a href="client_portalfinal.php" class="btn-cancel">
...
<script>
/* ══════════════════════════════════════════════════════════════════════
   BUSIQUIP FAULT REPORT — CLIENT-SIDE LOGIC
══════════════════════════════════════════════════════════════════════ */

// ── Live clock on strip ──────────────────────────────────────────────
setInterval(()=>{
  const now=new Date();
  document.getElementById('strip-time').textContent=
...
function toggleTheme(){
  ...
  ic.className=document.body.classList.contains('lm')?'fas fa-sun':'fas fa-moon';
...
function selectToggle(el,group){
  ...
  document.getElementById(
    group==='operational'?'is_operational':
    group==='recurrence'?'occurred_before':'service_visit'
  ).value=val;
}
...
function updateProgress(){
  const filled=[
    document.getElementById('reported_by').value.trim().length>1,
    document.getElementById('fault_title').value.trim().length>=5 &&
      document.getElementById('description').value.trim().length>=20 &&
      document.getElementById('fault_category').value,
    document.getElementById('equipment_type').value,
    document.getElementById('priority_val').value,
    true // attachments optional
  ];
  for(let i=1;i<=5;i++){
    const s=document.getElementById('step-'+i);
    const l=document.getElementById('line-'+i);
    if(filled[i-1]){s.classList.add('done');s.classList.remove('active')}
    else{s.classList.remove('done')}
    if(l && filled[i-1]) l.classList.add('done');
    else if(l) l.classList.remove('done');
  }
  const nextIdx=filled.findIndex(f=>!f);
  if(nextIdx>=0){
    document.getElementById('step-'+(nextIdx+1)).classList.add('active');
  }
  validateSubmitBtn();
}
...
function setupCounter(inputId,countId,max){
  ...
  const update=()=>{
    ...
    cnt.className='char-count'+(len>max*0.9?' warn':'')+(len>=max?' over':'');
  };
  inp.addEventListener('input',update);
}
setupCounter('fault_title','title-count',120);
setupCounter('description','desc-count',2000);
...
function validateField(el){
  ...
  if(el.id==='fault_title' && val.length>0 && val.length<5){ok=false}
  if(el.id==='description' && val.length>0 && val.length<20){ok=false}
  if(el.type==='date' && val){
    if(new Date(val)>new Date()) ok=false;
  }
  fg.classList.toggle('has-error',!ok && val.length>0 || (!ok && el.hasAttribute('required') && !val && el.dataset.touched));
  fg.classList.toggle('has-ok', ok && val.length>0);
  return ok;
}

document.querySelectorAll('input:not([readonly]):not([type=hidden]):not([type=file]),select,textarea').forEach(el=>{
  el.addEventListener('blur',()=>{el.dataset.touched='1';validateField(el);updateProgress();});
  el.addEventListener('input',()=>{if(el.dataset.touched)validateField(el);updateProgress();autoSave();});
});
...
function handleFiles(fileList){
  ...
  Array.from(fileList).forEach(f=>{
    if(!allowed.includes(f.type)){alert2('w','File type not allowed: '+f.name);return;}
    if(f.size>maxSize){alert2('w','File too large (max 20MB): '+f.name);return;}
...
function addPreview(file){
  ...
  rem.className='rem-btn';rem.innerHTML='×';rem.type='button';
  ...
  if(file.type.startsWith('image/')){
    ...
    const iconMap={'application/pdf':'fa-file-pdf','application/msword':'fa-file-word',
      'application/vnd.openxmlformats-officedocument.wordprocessingml.document':'fa-file-word',
      'text/plain':'fa-file-alt'};
    const ic=document.createElement('i');
    ic.className='fas '+(iconMap[file.type]||'fa-file')+' file-icon';
    ic.style.color='var(--gold)';
...
function autoSave(){
  ...
  autoTimer=setTimeout(()=>{
    const data={
      fault_title:document.getElementById('fault_title').value,
      description:document.getElementById('description').value,
      fault_category:document.getElementById('fault_category').value,
    ...
    try{sessionStorage.setItem('bq_draft',JSON.stringify(data));}catch(e){}
...
(function(){
  try{
    ...
    if(d.fault_title){
      ['fault_title','description','fault_category','equipment_type','brand_model','serial_number','fault_location','dept_branch','users_affected'].forEach(k=>{
        if(d[k]&&document.getElementById(k)) document.getElementById(k).value=d[k];
      });
      alert2('i','Draft restored from your previous session.');
    }
  }catch(e){}
})();

// ── Load client registered equipment ──────────────────────────────────
(function(){
  fetch('?action=get_client_products')
    ...
    .then(data=>{
      ...
      if(data.length===0){sel.innerHTML='<option value="">No registered equipment found</option>';return;}
      data.forEach(p=>{
        const o=document.createElement('option');
        o.value=p.CLIENT_PROD_ID;
        o.textContent=p.PROD_NAME+' ('+(p.SERIAL_NUM||'No serial')+')';
        sel.appendChild(o);
      });
    })
    .catch(()=>{
      document.getElementById('client_prod_id').innerHTML='<option value="">Could not load equipment</option>';
    });
})();
...
function validateSubmitBtn(){
  const required=[
    {id:'reported_by',min:2,label:'Reporter name'},
    {id:'fault_title',min:5,label:'Fault title (min 5 chars)'},
    {id:'description',min:20,label:'Fault description (min 20 chars)'},
    {id:'fault_category',label:'Fault category'},
    {id:'equipment_type',label:'Equipment type'},
    {id:'fault_date',label:'Date fault occurred'},
    {id:'fault_time',label:'Time fault occurred'},
  ];
  const errors=[];
  required.forEach(r=>{
    ...
    if(!el) return;
    const val=el.value.trim();
    if(!val||(r.min&&val.length<r.min)) errors.push(r.label);
    if(r.id==='fault_date'&&val&&new Date(val)>new Date()) errors.push('Fault date cannot be in the future');
  });
...
function alert2(type,msg){
  ...
  const icons={s:'check-circle',e:'circle-xmark',w:'triangle-exclamation',i:'info-circle'};
  al.className='al al-'+type;
  al.innerHTML='<i class="fas fa-'+icons[type]+'"></i><span>'+msg+'</span>';
  container.appendChild(al);
  setTimeout(()=>{al.style.opacity='0';setTimeout(()=>al.remove(),400);},5500);
}
...
async function submitFault(){
  ...
  const requiredFields=[
    {id:'reported_by',min:2},
    {id:'fault_title',min:5},
    {id:'description',min:20},
    {id:'fault_category',min:1},
    {id:'equipment_type',min:1},
    {id:'fault_date',min:1},
    {id:'fault_time',min:1},
  ];
  let valid=true;
  requiredFields.forEach(r=>{
    ...
    const val=el.value.trim();
    const fg=el.closest('.fg');
    if(!val||(r.min>1&&val.length<r.min)){
      valid=false;
      if(fg){fg.classList.add('has-error');fg.classList.remove('has-ok');}
    }
  });
  const dateEl=document.getElementById('fault_date');
  if(dateEl.value&&new Date(dateEl.value)>new Date()){
    valid=false;dateEl.closest('.fg').classList.add('has-error');
  }
  if(!valid){alert2('e','Please complete all required fields correctly before submitting.');return;}
  ...
  try{
    ...
    if(data.success){
      ...
      alert2('s','Fault submitted! Ref: '+data.ref);
    }else{
      alert2('e',data.error||'Submission failed. Please try again.');
      btn.disabled=false;
      btn.innerHTML='<i class="fas fa-paper-plane"></i> Submit Fault Report';
    }
  }catch(err){
    alert2('e','Network error. Please check your connection and try again.');
    btn.disabled=false;
    btn.innerHTML='<i class="fas fa-paper-plane"></i> Submit Fault Report';
  }
}
...
function resetForm(){
  ...
  // Reset toggles
  document.querySelectorAll('[data-group="operational"]').forEach(t=>t.classList.remove('selected-yes','selected-no','selected-neutral'));
  document.querySelector('[data-group="operational"][data-val="Yes"]').classList.add('selected-yes');
  document.querySelectorAll('[data-group="recurrence"]').forEach(t=>t.classList.remove('selected-yes','selected-no','selected-neutral'));
