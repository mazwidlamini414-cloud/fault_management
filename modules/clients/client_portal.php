...
body{font-family:var(--fb);background:var(--bg0);color:var(--t1);min-height:100vh;overflow-x:hidden;transition:background .4s,color .4s;font-size:15px}
...
.ticker-inner{display:flex;gap:70px;white-space:nowrap;animation:tick 28s linear infinite;
    font-family:var(--fm);font-size:13px;letter-spacing:.06em;color:var(--gold2)}
...
.brand-nm{font-family:var(--fh);font-size:24px;font-weight:800;
...
.brand-sub{font-size:11px;color:var(--t2);letter-spacing:.15em;text-transform:uppercase;font-family:var(--fm)}
...
.h-search input{flex:1;background:none;border:none;outline:none;color:var(--t1);
    font-family:var(--fb);font-size:15px;padding:9px 0}
...
.h-nm .n{font-weight:600;font-size:15px}
.h-nm .e{font-size:13px;color:var(--t2)}
...
.ni .nb{margin-left:auto;background:var(--dan);color:#fff;font-size:11px;
...
.exp-btn{display:flex;align-items:center;gap:11px;padding:10px 18px;margin:2px 8px;
    ...
    color:var(--gold);font-size:14px;font-weight:700;width:calc(100% - 16px);transition:var(--tr)}
...
.si{display:flex;align-items:center;gap:9px;padding:8px 14px 8px 42px;margin:2px 8px;
    border-radius:8px;color:var(--t3);font-size:14px;cursor:pointer;transition:var(--tr)}
...
.s-banner p{font-size:13px;color:var(--t2);line-height:1.5}
.s-banner a{display:inline-block;margin-top:10px;padding:6px 14px;border-radius:6px;
    background:linear-gradient(135deg,var(--burg),var(--gold));color:#fff;font-size:13px;font-weight:700;transition:var(--tr)}
...
.al{display:flex;align-items:center;gap:10px;padding:12px 18px;border-radius:var(--r);
    font-size:15px;font-weight:500;pointer-events:all;backdrop-filter:var(--blur);
...
.hero-greet{font-family:var(--fh);font-size:14px;font-weight:600;letter-spacing:.1em;
...
.hero h1{font-family:var(--fh);font-size:clamp(28px,3.5vw,42px);font-weight:800;line-height:1.15;
...
.hero p{font-size:16px;color:var(--t2);line-height:1.7;margin-bottom:20px;max-width:500px}
.chips{display:flex;flex-wrap:wrap;gap:9px}
.chip{display:inline-flex;align-items:center;gap:6px;padding:7px 15px;border-radius:99px;
    font-size:13px;font-weight:600;border:1px solid;backdrop-filter:var(--blur)}
...
.sn{font-family:var(--fh);font-size:38px;font-weight:800;
...
.sl{font-size:14px;color:var(--t2);letter-spacing:.03em}
.ss{font-size:12px;color:var(--t3);margin-top:5px;font-family:var(--fm)}
...
.ft{font-family:var(--fh);font-size:18px;font-weight:700;margin-bottom:7px}
.fd{font-size:14px;color:var(--t2);line-height:1.6}
.fb2{display:inline-flex;align-items:center;gap:6px;margin-top:16px;padding:8px 18px;
    ...
    border-radius:8px;font-size:14px;font-weight:600;transition:var(--tr)}
...
.st{font-family:var(--fh);font-size:20px;font-weight:800;display:flex;align-items:center;gap:9px}
...
.sl2{font-size:14px;color:var(--gold);font-weight:600;display:flex;align-items:center;gap:4px;cursor:pointer;transition:var(--tr)}
.sl2:hover{gap:8px}
...
.ai-t{font-weight:600;font-size:15px;margin-bottom:3px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.ai-m{font-size:12px;color:var(--t2);display:flex;gap:10px;flex-wrap:wrap}
.ai-m span{display:flex;align-items:center;gap:3px}

/* ═══ BADGES ════════════════════════════════════════════════════════ */
.b{display:inline-flex;align-items:center;gap:4px;padding:4px 10px;border-radius:99px;
...
.pr-l{font-size:14px;color:var(--t2)}
.pr-sub{font-size:12px;color:var(--t3);margin-top:2px}
.pr-amt{font-family:var(--fm);font-size:15px;font-weight:600;color:var(--gold)}
...
.mh h2{font-family:var(--fh);font-size:22px;font-weight:800;display:flex;align-items:center;gap:9px}
...
.mt{padding:9px 16px;background:none;border:none;color:var(--t2);font-size:14px;font-weight:600;
...
.fg label{font-size:13px;font-weight:600;color:var(--t2);letter-spacing:.04em;text-transform:uppercase}
.fg input,.fg textarea,.fg select{
    background:var(--bg3);border:1px solid var(--bor);color:var(--t1);
    padding:11px 14px;border-radius:8px;font-family:var(--fb);font-size:15px;transition:var(--tr);outline:none}
...
/* ═══ BUTTONS ═══════════════════════════════════════════════════════ */
.btn{display:inline-flex;align-items:center;gap:7px;padding:11px 22px;border-radius:8px;
    font-size:15px;font-weight:600;border:none;cursor:pointer;transition:var(--tr);flex-shrink:0}
...
.tbl th{padding:10px 14px;text-align:left;font-size:12px;text-transform:uppercase;
    letter-spacing:.07em;color:var(--t3);border-bottom:1px solid var(--bor);font-weight:600}
.tbl td{padding:13px 14px;font-size:14px;border-bottom:1px solid rgba(255,255,255,.03)}
.tbl tr:hover td{background:var(--gl)}
.tbl tr:last-child td{border-bottom:none}

/* ═══ DETAIL GRID ═══════════════════════════════════════════════════ */
.dg{display:grid;grid-template-columns:1fr 1fr;gap:10px}
.di{padding:12px 14px;background:var(--bg3);border:1px solid var(--bor);border-radius:8px}
.di .k{font-size:12px;color:var(--t3);text-transform:uppercase;letter-spacing:.06em;margin-bottom:3px}
.di .v{font-size:15px;font-weight:600}
.di.full{grid-column:1/-1}

/* ═══ INFO BOX ══════════════════════════════════════════════════════ */
.ib{padding:12px 14px;border-radius:8px;display:flex;gap:10px;align-items:flex-start;font-size:14px}
...
.empty p{color:var(--t2);font-size:15px}
...
@media(max-width:768px){
    ...
    footer{margin-left:0}.h-nm{display:none}
}
/* ═══ COMPANY LOGO (TOP LEFT, BIG) ═══════════════════════════════ */
.company-logo-img{
    height:90px;width:auto;max-width:240px;object-fit:contain;
    background:#fff;padding:6px 12px;border-radius:10px;
    box-shadow:0 4px 18px rgba(232,184,75,.35),0 2px 8px rgba(0,0,0,.35);
    flex-shrink:0;display:block;transition:all .28s cubic-bezier(.4,0,.2,1);
    margin-right:18px;order:-1}
.company-logo-img:hover{transform:scale(1.04);box-shadow:0 8px 28px rgba(232,184,75,.55)}
@media(max-width:768px){.company-logo-img{height:56px;max-width:140px;padding:4px 8px}}
</style>
<body>
...
<header>
    ...
    <div class="h-right">
        ...
        <button class="hb" onclick="openNotifications()" title="Notifications"><i class="fas fa-bell"></i></button>
...
<aside class="sidebar" id="sidebar">
    ...
    <div class="ni act" onclick="window.open('client_portal.php','_blank')"><div class="ic"><i class="fas fa-home"></i></div> Dashboard</div>
    ...
    <div class="ni" onclick="window.open('client_faults.php','_blank')"><div class="ic"><i class="fas fa-tools"></i></div> My Faults <span class="nb"><?php echo $f_total; ?></span></div>
    <div class="ni" onclick="window.open('client_products.php','_blank')"><div class="ic"><i class="fas fa-box-open"></i></div> My Products <span class="nb tl"><?php echo $prod_count; ?></span></div>

    <div class="slbl" style="margin-top:8px">Finance</div>
    <div class="ni" onclick="window.open('client_invoices.php','_blank')"><div class="ic"><i class="fas fa-receipt"></i></div> Invoices <?php if($i_unpaid>0): ?><span class="nb"><?php echo $i_unpaid; ?></span><?php endif; ?></div>
    <div class="ni" onclick="window.open('client_invoices.php','_blank')"><div class="ic"><i class="fas fa-credit-card"></i></div> Make Payment</div>
    ...
    <div class="sub-menu" id="sub-menu">
        <div class="si" onclick="openNotifications()"><i class="fas fa-bell"></i> Notifications</div>
        <div class="si" onclick="window.open('client_documents.php','_blank')"><i class="fas fa-folder"></i> Documents</div>
        <div class="si" onclick="openFeedback()"><i class="fas fa-star"></i> Leave Feedback</div>
        <div class="si" onclick="openHelp()"><i class="fas fa-life-ring"></i> Help & Support</div>
        <div class="si" onclick="openSettings()"><i class="fas fa-cog"></i> Settings</div>
    </div>
...
<!-- STATS -->
<div class="stats">
    <div class="sc" onclick="window.open('client_faults.php','_blank')">
    ...
    <div class="sc" onclick="window.open('client_faults.php?filter=Pending','_blank')">
    ...
    <div class="sc" onclick="window.open('client_faults.php?filter=Assigned','_blank')">
    ...
    <div class="sc" onclick="window.open('client_faults.php?filter=Resolved','_blank')">
    ...
    <div class="sc" onclick="window.open('client_invoices.php','_blank')">
    ...
    <div class="sc" onclick="window.open('client_products.php','_blank')">
...
<div class="feats">
    ...
    <div class="fc" onclick="window.open('client_faults.php','_blank')">
    ...
    <div class="fc" onclick="window.open('client_invoices.php','_blank')">
    ...
    <div class="fc" onclick="window.open('client_invoices','_blank')">
    ...
    <div class="fc" onclick="window.open('client_products.php','_blank')">
...
<div class="two">
    <div>
        <div class="sh">
            ...
            <span class="sl2" onclick="window.open('client_faults.php','_blank')">View all <i class="fas fa-arrow-right"></i></span>
        </div>
        <div class="acts">
        <?php
        $cnt=0;
        ...
        ?>
        <div class="ai" onclick="window.open('client_fault_detail.php?id=<?php echo $rf['REP_FAULT_ID']; ?>','_blank')" style="animation-delay:<?php echo $cnt*.07; ?>s">
        ...
        <div class="empty"><i class="fas fa-inbox"></i><p>No faults yet.</p>
            <button class="btn btn-p btn-sm" style="margin-top:12px" onclick="window.open('report_fault.php','_blank')"><i class="fas fa-plus"></i> Report First Fault</button>        </div>
        <?php endif; ?>
        </div>
    </div>
    ...
    <div>
        <div class="sh">
            ...
            <span class="sl2" onclick="window.open('client_invoices.php','_blank')">View all <i class="fas fa-arrow-right"></i></span>
        </div>
        <div class="pnl">
        <div class="pnl-b">
        <?php
...
<div class="mb" style="max-width:780px">
    ...
    <div class="mfoot">
        <button class="btn btn-p" onclick="window.open('report_fault.php','_blank')"><i class="fas fa-plus"></i> Report New Fault</button>
    </div>
</div>
</div>
...
<!-- NOTIFICATIONS -->
<div class="mo" id="m-notifications">
<div class="mb" style="max-width:620px">
    <div class="mh"><h2><i class="fas fa-bell" style="color:var(--gold)"></i>Notifications</h2><button class="mc" onclick="cm('m-notifications')">×</button></div>
    <div class="mbody" id="m-notif-body">
        <div class="ib ib-t"><i class="fas fa-info-circle" style="color:var(--teal)"></i>
            <div>Your latest system notifications and updates are shown below.</div>
        </div>
        <div style="display:grid;gap:10px;margin-top:8px">
            <div style="padding:14px 16px;background:var(--bg3);border:1px solid var(--bor);border-radius:10px;display:flex;gap:13px;align-items:flex-start">
                <div style="width:38px;height:38px;border-radius:9px;background:rgba(232,184,75,.15);display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:16px;color:var(--gold)"><i class="fas fa-wrench"></i></div>
                <div style="flex:1">
                    <div style="font-weight:600;font-size:14px;margin-bottom:3px">Fault Status Updates</div>
                    <div style="font-size:13px;color:var(--t2)">You will be notified here whenever a fault you reported changes status — from Pending to Assigned, In Progress or Resolved.</div>
                    <div style="font-size:11px;color:var(--t3);margin-top:5px;font-family:var(--fm)">Auto-updated · real-time</div>
                </div>
            </div>
            <div style="padding:14px 16px;background:var(--bg3);border:1px solid var(--bor);border-radius:10px;display:flex;gap:13px;align-items:flex-start">
                <div style="width:38px;height:38px;border-radius:9px;background:rgba(14,165,233,.15);display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:16px;color:var(--sky)"><i class="fas fa-file-invoice-dollar"></i></div>
                <div style="flex:1">
                    <div style="font-weight:600;font-size:14px;margin-bottom:3px">Invoice &amp; Payment Alerts</div>
                    <div style="font-size:13px;color:var(--t2)">Receive alerts when new invoices are issued, payments are confirmed, or when an invoice becomes overdue.</div>
                    <div style="font-size:11px;color:var(--t3);margin-top:5px;font-family:var(--fm)">Finance · billing</div>
                </div>
            </div>
            <div style="padding:14px 16px;background:var(--bg3);border:1px solid var(--bor);border-radius:10px;display:flex;gap:13px;align-items:flex-start">
                <div style="width:38px;height:38px;border-radius:9px;background:rgba(16,185,129,.15);display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:16px;color:var(--em)"><i class="fas fa-shield-alt"></i></div>
                <div style="flex:1">
                    <div style="font-weight:600;font-size:14px;margin-bottom:3px">System &amp; Security</div>
                    <div style="font-size:13px;color:var(--t2)">Important security alerts and system announcements from BUSIQUIP will appear here.</div>
                    <div style="font-size:11px;color:var(--t3);margin-top:5px;font-family:var(--fm)">System · security</div>
                </div>
            </div>
        </div>
    </div>
    <div class="mfoot"><button class="btn btn-s" onclick="cm('m-notifications')">Close</button></div>
</div>
</div>

<!-- LEAVE FEEDBACK -->
<div class="mo" id="m-feedback">
<div class="mb" style="max-width:580px">
    <div class="mh"><h2><i class="fas fa-star" style="color:var(--gold)"></i>Leave Feedback</h2><button class="mc" onclick="cm('m-feedback')">×</button></div>
    <div class="mbody">
        <div class="ib ib-g"><i class="fas fa-info-circle" style="color:var(--gold)"></i>
            <div>Your feedback helps us improve our service. We read every submission.</div>
        </div>
        <div class="fg">
            <label>How would you rate your experience?</label>
            <div id="star-rating" style="display:flex;gap:8px;font-size:28px;cursor:pointer;margin:4px 0">
                <span class="star" data-v="1" onclick="setRating(1)" style="color:var(--t3);transition:color .2s">★</span>
                <span class="star" data-v="2" onclick="setRating(2)" style="color:var(--t3);transition:color .2s">★</span>
                <span class="star" data-v="3" onclick="setRating(3)" style="color:var(--t3);transition:color .2s">★</span>
                <span class="star" data-v="4" onclick="setRating(4)" style="color:var(--t3);transition:color .2s">★</span>
                <span class="star" data-v="5" onclick="setRating(5)" style="color:var(--t3);transition:color .2s">★</span>
            </div>
        </div>
        ...
        <div class="fg"><label>Your Feedback *</label>
            <textarea id="fb-msg" placeholder="Share your experience, suggestions or concerns…" style="min-height:100px"></textarea>
        </div>
    </div>
    <div class="mfoot">
        <button class="btn btn-p" onclick="submitFeedback()"><i class="fas fa-paper-plane"></i> Submit Feedback</button>
        <button class="btn btn-s" onclick="cm('m-feedback')">Cancel</button>
    </div>
</div>
</div>

<!-- HELP & SUPPORT -->
<div class="mo" id="m-help">
<div class="mb" style="max-width:640px">
    <div class="mh"><h2><i class="fas fa-life-ring" style="color:var(--teal)"></i>Help &amp; Support</h2><button class="mc" onclick="cm('m-help')">×</button></div>
    <div class="mbody">
        <div style="display:grid;gap:12px">
            <div style="padding:16px;background:var(--bg3);border:1px solid var(--bor);border-radius:10px">
                <div style="font-family:var(--fh);font-weight:700;font-size:14px;margin-bottom:6px;color:var(--gold)"><i class="fas fa-headset"></i> Contact Support</div>
                <div style="font-size:13px;color:var(--t2);line-height:1.7">
                    Our support team is available <strong style="color:var(--t1)">Monday – Friday, 8AM – 5PM</strong> (Eswatini time).<br>
                    Email: <a href="mailto:support@busiquip.co.sz" style="color:var(--teal)">support@busiquip.co.sz</a><br>
                    Phone: <span style="color:var(--t1);font-family:var(--fm)">+268 2404 XXXX</span>
                </div>
            </div>
            <div style="padding:16px;background:var(--bg3);border:1px solid var(--bor);border-radius:10px">
                <div style="font-family:var(--fh);font-weight:700;font-size:14px;margin-bottom:10px;color:var(--gold)"><i class="fas fa-question-circle"></i> Frequently Asked Questions</div>
                <div style="display:grid;gap:8px">
                    <details style="background:var(--bg4);border-radius:8px;padding:10px 13px;cursor:pointer">
                        <summary style="font-size:13px;font-weight:600;color:var(--t1)">How do I report a new fault?</summary>
                        <p style="font-size:12px;color:var(--t2);margin-top:7px;line-height:1.6">Click <strong>Report Fault</strong> in the sidebar or the dashboard card. Fill in the description, priority and date, then click Submit. Our team will assign a technician as soon as possible.</p>
                    </details>
                    <details style="background:var(--bg4);border-radius:8px;padding:10px 13px;cursor:pointer">
                        <summary style="font-size:13px;font-weight:600;color:var(--t1)">How can I track my repair?</summary>
                        <p style="font-size:12px;color:var(--t2);margin-top:7px;line-height:1.6">Go to <strong>My Faults</strong> and click on any fault to see its current status, assigned technician and work log entries.</p>
                    </details>
                    <details style="background:var(--bg4);border-radius:8px;padding:10px 13px;cursor:pointer">
                        <summary style="font-size:13px;font-weight:600;color:var(--t1)">How do I make a payment?</summary>
                        <p style="font-size:12px;color:var(--t2);margin-top:7px;line-height:1.6">Navigate to <strong>Invoices</strong>, select an outstanding invoice, and use the payment form. Accepted methods include EFT, cash and mobile money.</p>
                    </details>
                    <details style="background:var(--bg4);border-radius:8px;padding:10px 13px;cursor:pointer">
                        <summary style="font-size:13px;font-weight:600;color:var(--t1)">I forgot my password. What do I do?</summary>
                        <p style="font-size:12px;color:var(--t2);margin-top:7px;line-height:1.6">Contact BUSIQUIP support via email or phone and we will reset your credentials securely.</p>
                    </details>
                </div>
            </div>
        </div>
    ...
    <div class="mfoot">
        <a href="mailto:support@busiquip.co.sz" class="btn btn-p"><i class="fas fa-envelope"></i> Email Support</a>
        <button class="btn btn-s" onclick="cm('m-help')">Close</button>
    </div>
</div>
</div>
...
<div class="mb" style="max-width:560px">
    ...
    <div class="mbody">
        <div style="display:grid;gap:14px">
            <!-- Appearance -->
            <div style="padding:16px;background:var(--bg3);border:1px solid var(--bor);border-radius:10px">
                <div style="font-family:var(--fh);font-weight:700;font-size:14px;margin-bottom:12px;color:var(--gold)"><i class="fas fa-palette"></i> Appearance</div>
                <div style="display:flex;align-items:center;justify-content:space-between">
                    <div>
                        <div style="font-size:14px;font-weight:600">Theme</div>
                        <div style="font-size:12px;color:var(--t3)">Toggle between dark and light mode</div>
                    </div>
                    <button class="btn btn-s btn-sm" onclick="togTheme();updateThemeBtn()" id="theme-toggle-btn">
            ...
            <div style="padding:16px;background:var(--bg3);border:1px solid var(--bor);border-radius:10px">
                <div style="font-family:var(--fh);font-weight:700;font-size:14px;margin-bottom:12px;color:var(--gold)"><i class="fas fa-bell"></i> Notification Preferences</div>
                <div style="display:grid;gap:10px">
                    <label style="display:flex;align-items:center;justify-content:space-between;cursor:pointer">
                        <div><div style="font-size:13px;font-weight:600">Fault status updates</div><div style="font-size:11px;color:var(--t3)">Notify me when my fault status changes</div></div>
                        <input type="checkbox" id="notif-fault" checked style="width:18px;height:18px;accent-color:var(--gold)">
                    </label>
                    <label style="display:flex;align-items:center;justify-content:space-between;cursor:pointer">
                        <div><div style="font-size:13px;font-weight:600">Invoice alerts</div><div style="font-size:11px;color:var(--t3)">New invoices and payment confirmations</div></div>
                        <input type="checkbox" id="notif-inv" checked style="width:18px;height:18px;accent-color:var(--gold)">
                    </label>
                    <label style="display:flex;align-items:center;justify-content:space-between;cursor:pointer">
                        <div><div style="font-size:13px;font-weight:600">System announcements</div><div style="font-size:11px;color:var(--t3)">General system and maintenance notices</div></div>
                        <input type="checkbox" id="notif-sys" style="width:18px;height:18px;accent-color:var(--gold)">
                    </label>
                </div>
            </div>
            <!-- Account -->
            <div style="padding:16px;background:var(--bg3);border:1px solid var(--bor);border-radius:10px">
                <div style="font-family:var(--fh);font-weight:700;font-size:14px;margin-bottom:12px;color:var(--gold)"><i class="fas fa-user-cog"></i> Account</div>
                <div style="font-size:13px;color:var(--t2);margin-bottom:12px">To update your contact details, company information or password, please contact BUSIQUIP support.</div>
                <a href="mailto:support@busiquip.co.sz" class="btn btn-s btn-sm"><i class="fas fa-envelope"></i> Request Account Update</a>
            </div>
        </div>
    </div>
    <div class="mfoot">
        <button class="btn btn-p" onclick="saveSettings()"><i class="fas fa-save"></i> Save Settings</button>
        <button class="btn btn-s" onclick="cm('m-settings')">Close</button>
    </div>
</div>
</div>
...
function setTheme(t){document.body.className=t==='light'?'lm':'';localStorage.setItem('bq-theme',t);curTheme=t;}
...
async function loadFaultDetail(id){
    ...
    `;
    document.getElementById('fd-foot').innerHTML=`
        <button class="btn btn-s" onclick="window.open('client_faults.php','_blank')"><i class="fas fa-arrow-left"></i> Back to Faults</button>
    `;
}
...
/* ── NEW PANEL OPENERS ──────────────────────────────────────────── */
function openNotifications(){ om('m-notifications'); }
function openFeedback(){ om('m-feedback'); }
...
function updateThemeBtn(){
    ...
    if(!btn)return;
    if(curTheme==='dark'){btn.innerHTML='<i class="fas fa-sun"></i> Light Mode';}
    else{btn.innerHTML='<i class="fas fa-moon"></i> Dark Mode';}
}
...
function setRating(v){
    ...
    document.querySelectorAll('.star').forEach(s=>{
        s.style.color=parseInt(s.dataset.v)<=v?'var(--gold)':'var(--t3)';
    });
}
...
function submitFeedback(){
    ...
    if(!msg){alert2('e','Please enter your feedback message.');return;}
    if(!fbRating){alert2('e','Please select a star rating.');return;}
    alert2('s','Thank you for your feedback! We appreciate your input.');
    cm('m-feedback');
...
function saveSettings(){
    localStorage.setItem('bq-notif-fault',document.getElementById('notif-fault').checked?'1':'0');
    localStorage.setItem('bq-notif-inv',document.getElementById('notif-inv').checked?'1':'0');
    localStorage.setItem('bq-notif-sys',document.getElementById('notif-sys').checked?'1':'0');
    alert2('s','Settings saved successfully.');
    cm('m-settings');
}

/* ── RESTORE SETTINGS ────────────────────────────────────────────── */
document.addEventListener('DOMContentLoaded',function(){
    const f=localStorage.getItem('bq-notif-fault');
    const i=localStorage.getItem('bq-notif-inv');
    const s=localStorage.getItem('bq-notif-sys');
    if(f!==null){const el=document.getElementById('notif-fault');if(el)el.checked=f==='1';}
    if(i!==null){const el=document.getElementById('notif-inv');if(el)el.checked=i==='1';}
    if(s!==null){const el=document.getElementById('notif-sys');if(el)el.checked=s==='1';}
});
